...
If the user specifies a different install path, use that instead of `~/tradingview-autopilot`.
...
```bash
cp ~/tradingview-autopilot/rules.example.json ~/tradingview-autopilot/rules.json
```
