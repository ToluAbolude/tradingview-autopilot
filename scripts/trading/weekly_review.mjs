/**
 * weekly_review.mjs â€” Last-week trade audit
 *
 * For each major instrument Ã— timeframe:
 *   1. Pull bars covering last week
 *   2. Filter to Sun 22:00 UTC â†’ Fri 22:00 UTC
 *   3. Detect all signals (BOS-retest, EMA-stack, SmartTrail, etc.)
 *   4. Simulate virtual trades (SL=1.5Ã—ATR, TP=3Ã—ATR, 1:2 RR)
...
// â”€â”€ Last week window â”€â”€
// Today is Friday 2026-04-24. Last week = Sun 2026-04-13 22:00 UTC â†’ Fri 2026-04-18 22:00 UTC
...
// Timeframes to analyse (label â†’ TradingView resolution string)
const TIMEFRAMES = [
  { tf: '15',  label: '15M' },
  { tf: '60',  label: 'H1'  },
  { tf: '240', label: 'H4'  },
...
// â”€â”€ Indicators (copied from setup_finder) â”€â”€
...
// â”€â”€ Signal detection: returns true/false for long/short entry at bar i â”€â”€
function detectSignals(bars, i) {
  ...
  // Retest of BOS level (within 0.5Ã—ATR)
  ...
  const reasons = [];
  if (emaLong)    reasons.push('EMAâ†‘');
  if (stLong)     reasons.push('STâ†‘');
  if (retestLong) reasons.push('BOS-retestâ†‘');
  if (emaShort)   reasons.push('EMAâ†“');
  if (stShort)    reasons.push('STâ†“');
  if (retestShort)reasons.push('BOS-retestâ†“');
...
// â”€â”€ Virtual trade outcome: given entry at bar[i], scan forward for SL/TP hit â”€â”€
...
// â”€â”€ Trend context: was instrument in a clear trend during the week? â”€â”€
...
report.push('  WEEKLY TRADE AUDIT â€” 2026-04-13 22:00 UTC â†’ 2026-04-18 22:00 UTC');
report.push('â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•');
report.push('');

for (const inst of INSTRUMENTS) {
  report.push(`\nâ”Œâ”€â”€ ${inst.label} â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€`);
  const instTrades = [];
  const instSummary = {};
  ...
  for (const { tf, label: tfLabel } of TIMEFRAMES) {
    ...
    if (!bars || bars.length < 60) {
      report.push(`â”‚  [${tfLabel}] no data`);
      continue;
    }

    const weekBars = bars.filter(b => b.t >= WEEK_START && b.t <= WEEK_END);
    if (weekBars.length < 3) {
      report.push(`â”‚  [${tfLabel}] insufficient bars in window (${weekBars.length})`);
      continue;
    }
    ...
    report.push(`â”‚`);
    report.push(`â”‚  [${tfLabel}] ${weekBars.length} bars | Trend: ${trend} | Move: ${weekMove}% | H:${fmt(weekHigh,2)} L:${fmt(weekLow,2)}`);
    ...
    if (tf === '15' || tf === '60') {
      ...
      report.push(`â”‚  [${tfLabel}] Signals: ${total} | W:${wins} L:${losses} Open:${opens} | WR: ${wr}%`);

      if (signals.length > 0) {
        report.push(`â”‚  [${tfLabel}] Trades:`);
        for (const s of signals.slice(0, 12)) {
          const icon = s.outcome.result === 'WIN' ? 'âœ…' : s.outcome.result === 'LOSS' ? 'âŒ' : 'â³';
          report.push(`â”‚    ${icon} ${s.ts} ${s.dir.toUpperCase()} | ${s.outcome.result} in ${s.outcome.bars}bars | entry:${fmt(s.outcome.entry,2)} sl:${fmt(s.outcome.sl,2)} tp:${fmt(s.outcome.tp,2)} | RSI:${Math.round(s.rsi)}`);
        }
      }
      ...
      report.push(`â”‚  [${tfLabel}] Price: ${ctx} | Week range: ${fmt(weekHigh-weekLow,2)} pts`);
    }
  }

  // Instrument summary
  report.push(`â”‚`);
  report.push(`â”‚  SUMMARY for ${inst.label}:`);
  for (const [tfLabel, s] of Object.entries(instSummary)) {
    report.push(`â”‚    ${tfLabel}: ${s.trend} | ${s.total} signals | WR ${s.wr}% (${s.wins}W/${s.losses}L)`);
  }

  // Missed big moves: was there a 3Ã—ATR+ move with no signal at the start?
  try {
    ...
    if (h1bars) {
      ...
      if (wb.length >= 5) {
        ...
        if (maxBullRun > atrAtStart * 3) report.push(`â”‚  ðŸ“ˆ Biggest bull run: ${fmt(maxBullRun,2)} (${(maxBullRun/atrAtStart).toFixed(1)}Ã—ATR) starting ~${maxBullStart}`);
        if (maxBearRun > atrAtStart * 3) report.push(`â”‚  ðŸ“‰ Biggest bear run: ${fmt(maxBearRun,2)} (${(maxBearRun/atrAtStart).toFixed(1)}Ã—ATR) starting ~${maxBearStart}`);
      }
    }
  } catch(_) {}
...
report.push('â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•');
...
if (sorted.length) {
  ...
  report.push(`  â†’ Prioritise: ${best.join(', ')}`);
}
...
report.push(`  â†’ Long WR: ${bullWR}% (${bullTrades.length} trades) | Short WR: ${bearWR}% (${bearTrades.length} trades)`);
if (bullWR > bearWR + 10) report.push('  â†’ This week favoured LONGS strongly â€” consider raising bar for shorts');
if (bearWR > bullWR + 10) report.push('  â†’ This week favoured SHORTS strongly â€” consider raising bar for longs');
