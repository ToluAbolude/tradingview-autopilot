...
// Retry + rejected-promise-reset now live inside bridge.connect() itself
// (2026-08-18), so every caller gets them — not just this report.
...
function buildHtml(label, trades, sum, equity, openPos = [], tvoSec = null) {
...
async function pullTvoSection(fromMs) {
  // The cron generates this report TWICE (experiment env, then scanner env) and
  // stacks both into one email — so an unconditional Tradovate block appeared
  // twice. It is one account, not per-cTrader-account data: the experiment run
  // passes TVO_SECTION=off and only the scanner run (which is last) renders it,
  // putting it exactly once at the end of the email.
  if ((process.env.TVO_SECTION ?? 'on') === 'off') return null;
...
// Rendered in the SAME visual language as the cTrader sections above (section
// heading + #f6f8fa table header + row separators) and placed INSIDE the 640px
// card. The old version emitted a bare <h3> and a border="1" table injected
// after </body>, so it landed outside the card looking like a different email.
function tvoHtml(t) {
  if (!t) return '';
  const head = (sub) => `<tr><td style="padding:16px 24px 4px;font-size:15px;font-weight:700;color:#1e2a38">Tradeify Futures <span style="font-weight:400;color:#90a4ae;font-size:13px">— ${sub}</span></td></tr>`;

  if (t.error) {
    return head('Tradovate prop account') +
      `<tr><td style="padding:0 24px 8px;font-size:13px;color:#c62828">⚠ status unavailable: ${esc(t.error)}</td></tr>`;
  }

  // Headroom to the trailing-DD floor is the number that ends the account.
  const hrColor = t.headroom <= 250 ? '#c62828' : t.headroom <= 500 ? '#ef6c00' : '#2e7d32';
  const status = `<tr><td style="padding:0 24px 8px;font-size:13px;color:#555">` +
    `Equity <b>${money(t.equity)}</b> &nbsp;·&nbsp; DD floor ≈ ${money(t.floorEst)} &nbsp;·&nbsp; ` +
    `headroom <b style="color:${hrColor}">${money(t.headroom)}</b>` +
    (t.positions.length ? ` &nbsp;·&nbsp; open: ${esc(t.positions.map(p => `${p.contract} ${p.netPos > 0 ? '+' : ''}${p.netPos}`).join(', '))}` : '') +
    `</td></tr>`;

  if (!t.trades.length) {
    return head('Tradovate prop account') + status +
      `<tr><td style="padding:0 24px 8px;font-size:13px;color:#90a4ae">No futures signals routed today.</td></tr>`;
  }

  const rows = t.trades.map(s => {
    if (!s.tvo.placed) {
      return `<tr style="border-bottom:1px solid #eee">
        <td style="padding:7px 12px;color:#555">${esc(s.ts.slice(11, 16))}</td>
        <td style="padding:7px 12px;font-weight:600">${esc(s.symbol)}</td>
        <td style="padding:7px 12px;text-transform:uppercase;color:#555;font-size:12px">${esc(s.dir)}</td>
        <td style="padding:7px 12px;color:#90a4ae;font-size:12px" colspan="3">skipped: ${esc(s.tvo.error)}</td>
      </tr>`;
    }
    return `<tr style="border-bottom:1px solid #eee">
      <td style="padding:7px 12px;color:#555">${esc(s.ts.slice(11, 16))}</td>
      <td style="padding:7px 12px;font-weight:600">${esc(s.symbol)}</td>
      <td style="padding:7px 12px;text-transform:uppercase;color:#555;font-size:12px">${esc(s.dir)}</td>
      <td style="padding:7px 12px;text-align:right">${esc(s.tvo.units)}x @ ${esc(s.tvo.fill)}</td>
      <td style="padding:7px 12px;text-align:right;font-size:12px"><span style="color:#c62828">${esc(s.tvo.sl)}</span> / <span style="color:#2e7d32">${esc(s.tvo.tp)}</span></td>
      <td style="padding:7px 12px;text-align:right;color:#555">${money(Math.round(s.tvo.riskUsd))}</td>
    </tr>`;
  }).join('');
  ...
  return head('Tradovate prop account') + status +
    `<tr><td style="padding:0 18px 4px">
      <table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;font-size:14px">
        <tr style="background:#f6f8fa;color:#555;font-size:12px;text-transform:uppercase">
          <td style="padding:7px 12px">Time</td><td style="padding:7px 12px">Contract</td>
          <td style="padding:7px 12px">Dir</td><td style="padding:7px 12px;text-align:right">Fill</td>
          <td style="padding:7px 12px;text-align:right">SL / TP</td><td style="padding:7px 12px;text-align:right">Risk</td>
        </tr>
        ${rows}
      </table>
    </td></tr>`;
...
async function main() {
  ...
  writeFileSync(HTML_FILE, buildHtml(label, trades, sum, equity, openPos, tvoSec));
  console.log('\nHTML → ' + HTML_FILE);
