...
#   1. Restart tv_browser after 3 consecutive CDP failures; if plain restarts are
...
#   2. EMAIL after ~30 min of continuous downtime (was ~1h). The scanner's
...
# ── 1. CDP health: healthy resets all state ──
...
# ── 2. restart, with hard-reset escalation when restarts aren't sticking ──
