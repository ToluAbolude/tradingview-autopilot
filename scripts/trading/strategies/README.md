...
| `target` | yes | `{ "r": 2 }`: take profit at r × risk, unless the signal brings its own `tp`. |
| `risk` | yes | `{ "per_trade_pct": 0.1 }`: percent of equity lost if the stop is hit (at most 2). |
...
- `assertOrderSafety` on every order: stop-distance floors, lot caps, frozen-price check, fib veto — and on the scanner account (2118552), the daily plan gate, so orders there must sit at a planned zone inside the trade windows.
...
- Strategies on the scanner account (2118552): the logic works, but no cron line runs `strategy_runner.mjs` with the scanner env yet.
